...
/// Switch the active onboard DPI level (onboard mode only) and read back.
pub fn apply_level_select(r: &HidReceiver, d: &mut Device, level: u8, value: u16) -> Result<String, String> {
    r.link.set_current_dpi_index(d.index, level).map_err(|e| format!("set level: {e}"))?;
    let now = r.link.current_dpi_index(d.index).map_err(|e| format!("read back: {e}"))?;
    logger::log(format!("onboard dpi level: slot {} → {level} (device reports {now})", d.index));
    if now != level {
        return Err(format!("device kept level {now} instead of {level}"));
    }
